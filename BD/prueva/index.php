        ...
        <form id="formFormatoGS" onsubmit="GuardarRes(); return false;">
            <table>
                <thead >
                    <tr id="cabecera">    
                        <th class="bg-vo" >#</th>
                        <th class="bg-vo">Pregunta </th>
                        <th class="bg-vo" style="font-size:12px">SI (SI) | NO (NO) | OTRA (O)</th>
                    </tr>
                </thead>
                <tbody>
                <?php 
        ...
        <p class="conteFlex">
            <?php if ($responseType == 1) { ?>
                ...
                <?php foreach ($arrayRespuestas as $clave => $valor) { ?>
                    <span class="class_<?php echo $questionId; ?>" id="spanId_<?php echo $questionId . '_' . $clave; ?>" onclick="respuesta('<?php echo $questionId; ?>','<?php echo $clave; ?>')">
                        <input type="radio" name="respuesta[<?php echo $questionId; ?>]" id="idResp_<?php echo $questionId . $clave; ?>" value="<?php echo $clave; ?>">
                        <label for="idResp_<?php echo $questionId . $clave; ?>">
                            <?php echo ($valor); ?>
                        </label>
                    </span>
                <?php } ?>
            <?php } elseif ($responseType == 4) { ?>
                ...
                <?php if ($dataRow['respuesta_abierta'] == 0) { ?>
                    <textarea name="respuesta[<?php echo $questionId; ?>]" id="idResp_<?php echo $questionId; ?>"></textarea>
                <?php } else { ?>
                    <input type="text" name="respuesta[<?php echo $questionId; ?>]" id="idResp_<?php echo $questionId; ?>">
                <?php } ?>
            <?php } ?>
        </p>
    </td>
